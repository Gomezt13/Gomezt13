...
ROWS = [
    ("Subject", "Euler Gómez"),
    ("Role", "Agronomist · Innovation & Development"),
    ("Origin", "Perú"),
    ("Education", "Agricultural Engineering"),
    ("Status", "Researching + Learning + Building"),
    ("ToolChain", "Python · R · Git · GitHub"),
    ("Core.Agro", "Crop Science · Plant Protection"),
    ("Core.AI", "Computer Vision · Machine Learning"),
    ("Core.Data", "Statistics · Data Analysis"),
    ("Core.GIS", "QGIS · Spatial Analysis"),
    ("Core.Research", "Experiments · Data · Innovation"),
    ("Grid.Mail", "—"),
    ("Grid.LinkedIn", "/in/egomeztorres"),
    ("Grid.GitHub", "Gomezt13"),
    ("Grid.X", "—"),
]
...
def make_logos() -> dict[str, Image.Image]:
    """Load logo PNGs used as particle silhouettes."""
    ...
    for name, path in logo_files.items():
        if not path.exists():
            raise FileNotFoundError(
                ...
                f"Place {name}.png inside scripts/banner/logos/"
            )

        image = Image.open(path).convert("RGBA")

        # Put the logo on a transparent 400x400 canvas
...
def render_svg(
    ...
    python = transport(source, logo_points["python"][:n])
    r = transport(python, logo_points["r"][:n])
    github = transport(r, logo_points["github"][:n])
    ...
    parts: list[str] = [
        ...
        "<title id=\"title\">Euler's live system profile</title>",
        '<desc id="desc">Animated terminal profile with a dithered portrait and '
        "Python, R, and GitHub silhouettes.</desc>",
        "<defs>",
    ...
    python_centroid = python.mean(axis=0)
